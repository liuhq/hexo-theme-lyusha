if (!(hexo.config.archive && hexo.config.archive.enabled === false)) {
    ...
    hexo.config.archive_generator = Object.assign({
        archive_all_per_page: false, // NOTE:: Close the first page of the archive page for pagination
        per_page: per_page,
        yearly: true,
        monthly: false,
        daily: false
    ...
    function generatorArchive (locals) {
        const config = this.config;
        let archiveDir = config.archive_dir;
        const paginationDir = config.pagination_dir || 'page';
        const archiveAllPerPage = config.archive_generator.archive_all_per_page || false; // NOTE:: Close the first page of the archive page for pagination
        ...
        if (archiveAllPerPage) generate(archiveDir, allPosts); // NOTE:: Close the first page of the archive page for pagination
        ...
        for (let i = 0, len = years.length; i < len; i++) {
            ...
            // NOTE:: Disable "Monthly" and "Daily" folder grouping
    ...
    hexo.extend.generator.register('archive', generatorArchive);
}